#!/bin/bash
# Full OOD evaluation for the imagenet_ecoset136 norm experiment.
# Evaluates all 5 checkpoints, then runs cross-model error consistency.
#
# Usage:
#   ./scripts/run_full_ood_eval_imagenet_ecoset.sh
#   ./scripts/run_full_ood_eval_imagenet_ecoset.sh --datasets imagenet_r imagenet_a imagenet_c
...
echo "============================================"
echo "Full OOD Evaluation — ImageNet-Ecoset136"
echo "============================================"
echo ""

for norm in "${NORMS[@]}"; do
    run_dir="${LOG_DIR}/resnet50_${norm}_imagenet_ecoset"
...
RUN_DIRS=()
for norm in "${NORMS[@]}"; do
    pred_dir="${LOG_DIR}/resnet50_${norm}_imagenet_ecoset/predictions"
    if [ -d "$pred_dir" ] && [ "$(ls -A "$pred_dir" 2>/dev/null)" ]; then
        RUN_DIRS+=("${LOG_DIR}/resnet50_${norm}_imagenet_ecoset")
    fi
done
...
if [ ${#RUN_DIRS[@]} -ge 2 ]; then
    ...
    python scripts/analyze_error_consistency.py \
        --runs "${RUN_DIRS[@]}" \
        --output_dir "${LOG_DIR}/error_consistency_imagenet_ecoset"
else
